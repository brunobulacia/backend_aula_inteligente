...
class Command(BaseCommand):
    help = "Pobla la gestión 2-2024 para 1 alumno ya inscrito en Primero A (gestión 1-2024), para prueba segura."
    ...
    def handle(self, *args, **kwargs):
        ...
        # Tomar 1 alumno real de gestión 1
        mig_1 = MateriasInscritasGestion.objects.filter(gestion_curso=gc1).first()
        alumno = mig_1.ficha.matricula.alumno
        matricula = mig_1.ficha.matricula

        # Verificar si ya fue poblado
        if FichaInscripcion.objects.filter(matricula=matricula, materiasinscritasgestion__gestion_curso=gc2).exists():
            self.stdout.write(self.style.WARNING("⚠️ Este alumno ya tiene datos en gestión 2. Cancelando..."))
            return

        self.stdout.write(f"👨 Poblando gestión 2-2024 para el alumno: {alumno.nombre} {alumno.apellidos}")

        # Crear ficha nueva para gestión 2
        ficha2 = FichaInscripcion.objects.create(matricula=matricula)

        for asignacion in asignaciones2:
            materia = asignacion.materia

            # Buscar comportamiento anterior
            ficha1 = FichaInscripcion.objects.filter(matricula=matricula).order_by('id').first()
            ...
            # Evitar que las notas bajen de 0
        ...
        self.stdout.write(self.style.SUCCESS("✅ Gestión 2-2024 poblada para 1 alumno de prueba."))
